...
import threading
p=multiprocessing.Process(target=playsound, args=("Data\\3.mp3",))
def splash():
    splash=Tk()
    ...
    def main_window():
        ...
        def support():
            ...
            webbrowser.open("https://www.youtube.com/channel/UCyccSenXfhVEfqyVzWkcJkw")
        def about():
            playsound("Data\\1.mp3")
        ...
        def info():
            ...
            label_info.pack()
            label_info1=customtkinter.CTkLabel(master=root1,text=f"Machine type: {platform.machine()}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info1.pack()
            label_info2=customtkinter.CTkLabel(master=root1,text=f"Processor type: {platform.processor()}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info2.pack()
            label_info3=customtkinter.CTkLabel(master=root1,text=f"Number of physical cores: {psutil.cpu_count(logical=False)}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info3.pack()
            label_info4=customtkinter.CTkLabel(master=root1,text=f"Number of logical cores: {psutil.cpu_count(logical=True)}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info4.pack()
            label_info5=customtkinter.CTkLabel(master=root1,text=f"Current CPU frequency: {psutil.cpu_freq().current}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info5.pack()
            label_info6=customtkinter.CTkLabel(master=root1,text=f"Min CPU frequency: {psutil.cpu_freq().min}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info6.pack()
            label_info7=customtkinter.CTkLabel(master=root1,text=f"Max CPU frequency: {psutil.cpu_freq().max}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info7.pack()
            label_info8=customtkinter.CTkLabel(master=root1,text=f"Current CPU utilization: {psutil.cpu_percent(interval=1)}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info8.pack()
            label_info9=customtkinter.CTkLabel(master=root1,text=f"Current per-CPU utilization: {psutil.cpu_percent(interval=1, percpu=True)}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info9.pack()
            label_info10=customtkinter.CTkLabel(master=root1,text=f"Total RAM installed: {round(psutil.virtual_memory().total/1000000000, 2)} GB",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info10.pack()
            label_info11=customtkinter.CTkLabel(master=root1,text=f"Available RAM: {round(psutil.virtual_memory().available/1000000000, 2)} GB",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info11.pack()
            label_info12=customtkinter.CTkLabel(master=root1,text=f"Used RAM: {round(psutil.virtual_memory().used/1000000000, 2)} GB",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info12.pack()
            label_info13=customtkinter.CTkLabel(master=root1,text=f"RAM usage: {psutil.virtual_memory().percent}%",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info13.pack()
            label_info14=customtkinter.CTkLabel(master=root1,text=f"Operating system: {platform.system()}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info14.pack()
            label_info15=customtkinter.CTkLabel(master=root1,text=f"Operating system version: {platform.version()}",text_color="#7fe3c5",text_font=("Neon Zone",15,BOLD))
            label_info15.pack()
            root1.mainloop()
        def disable_event():
            pass
        ...
        lbl1=customtkinter.CTkLabel(master=window,text="beta-preview",text_color="#00ddff",text_font=("Zian",15))
        lbl1.pack()
        ...
        def a():
            ...
            window.destroy()
            p.terminate()
        exit_button=customtkinter.CTkButton(master=window,text="exit",text_font=("HACKED",15),command=a,fg_color="black",border_color="#700101",hover_color="#700101",border_width=1.5)
        exit_button.config(width=20,heigh=20,cursor="plus")
        exit_button.pack(side=BOTTOM,anchor=SE)
        window.mainloop()
    splash.after(0,main_window)
    splash.mainloop()
